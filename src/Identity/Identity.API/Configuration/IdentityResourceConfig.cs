...
{
    /*
	 * Declare Resource that is managed by IdentityServer4
		2 type of Resource
			- API Resource
			- Identity Resource
		Scope: Defined permission for api can do
	 */
        ...
        public static IEnumerable<Client> GetClients(Dictionary<string, string> clientUrls)
        {
            return new List<Client>()
            {
                new Client
                {
                    ...
                    RequireConsent = false, // false => Automation grand scopes | true: user have to choose scopes 
                    AllowOfflineAccess = true,
                    AlwaysIncludeUserClaimsInIdToken = true,
                    // Call back after login success
                    RedirectUris = new List<string>
                        {
                            $"{clientUrls["ExamWebApp"]}/authentication/login-callback"
                        },

                    // Call back whenever logout from IdentityServer
